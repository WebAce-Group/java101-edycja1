...
- W projekcie utwórz plik ```w1.Zadanie1.java```
  > Uwaga! Nazwa pliku musi być taka sama jak nazwa klasy!
  > W tym przypadku nazwa klasy to ```w1.Zadanie1```
  >
  > W klasie ```w1.Zadanie1``` utwórz metodę ```main```.
  > Plik powinien wyglądać tak:
  > ```java
  >public class w1.Zadanie1 {
...
## Wysyłanie rozwiązań
- Dodać pliki ```w1.Zadanie1.java``` i ```Zadanie2.java``` do repozytorium (```git add```).
